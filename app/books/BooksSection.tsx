...
import { getBooks } from "@/sanity/utils/books-utils";
...
export default function BooksSection() {
	...
	return (
		<section
			...
			aria-label="A list of all the books written by Javed Rasin"
		>
			<div className="flex flex-col items-center mb-10">
				<div className="flex flex-col gap-y-2 mt-20 mx-5">
					...
					>
						Books by Javed Rasin
					</h1>

					<p className="max-w-[30rem] text-center text-sm text-stone-800">
						A collection of all the books written by Javed Rasin.
					</p>
				</div>
				...
				{!isLoading && !isError && booksData.length === 0 && (
					<p className="mt-20">No books available for now.</p>
				)}

				{/* Books available. Show books grid and pagination */}
				{!isLoading && !isError && booksData.length > 0 && (
					<>
						{/* Books grid */}
						<BooksGrid currentPage={currentPage} booksData={booksData} />
...
function BooksGrid({
	...
	return (
		<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-x-7 gap-y-12 w-full my-20 px-5">
			{displayedBooks.map((book) => (
				<BookCard
					key={book._id}
					name={book.name}
					cover={book.bookCover.asset.url}
					alt={book.bookCover.alt}
...
function BookCard({
	...
	return (
		<Link href={`/books/${href}`}>
			...
			<div className="flex flex-col gap-y-6 group">
				<img
					src={cover}
					alt={alt}
					fetchPriority="high"
				...
				<div className="flex flex-col gap-y-1">
					<p className="text-lg font-medium text-stone-900">{name}</p>
